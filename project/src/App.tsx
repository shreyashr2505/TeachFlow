...
import PricingManagement from './components/Management/PricingManagement';
...
type SuperAdminView = 'overview' | 'classes' | 'payments' | 'pricing' | 'ai_control' | 'growth' | 'users';

const getSuperAdminView = (tab: string): SuperAdminView => {
  if (tab === 'dashboard') {
    return 'overview';
  }

  if (tab === 'classes' || tab === 'payments' || tab === 'pricing' || tab === 'ai_control' || tab === 'growth' || tab === 'users') {
    return tab;
  }
...
const isTiltLocked = (element: HTMLElement) => element.closest('[data-no-tilt="true"]') !== null;

const getTiltCard = (target: EventTarget | null) => {
  if (!(target instanceof Element)) return null;
  const card = target.closest('.app-motion :is(.rounded-xl, .rounded-2xl, .rounded-3xl, .rounded-lg, [class*="rounded-"])') as HTMLElement | null;
  if (!card || isTiltLocked(card)) return null;
  return card;
};

const updateTiltState = (card: HTMLElement, event: PointerEvent) => {
  ...
  card.style.setProperty('--tf-tilt-x', `${tiltX.toFixed(2)}deg`);
  card.style.setProperty('--tf-tilt-y', `${tiltY.toFixed(2)}deg`);
  card.style.setProperty('--tf-glow-x', `${(x * 100).toFixed(2)}%`);
  card.style.setProperty('--tf-glow-y', `${(y * 100).toFixed(2)}%`);
};
...
const TenantAppShell: React.FC = () => {
  ...
  const renderContent = () => {
    if (user?.role === 'super_admin') {
      return <SuperAdminDashboard view={getSuperAdminView(activeTab)} />;
    }
...
function App() {
  ...
  useEffect(() => {
    const handlePointerOver = (event: PointerEvent) => {
      ...
      if (!card) return;
      card.classList.add('tf-motion-active');
      updateTiltState(card, event);
    };

    const handlePointerMove = (event: PointerEvent) => {
      if (event.pointerType !== 'mouse') return;
      const card = getTiltCard(event.target);
      if (!card || !card.classList.contains('tf-motion-active')) return;
      updateTiltState(card, event);
    };
    ...
    const handlePointerOut = (event: PointerEvent) => {
      if (event.pointerType !== 'mouse') return;
      const card = getTiltCard(event.target);
      const related = getTiltCard(event.relatedTarget);
      if (!card || card === related) return;
      card.classList.remove('tf-motion-active');
      card.style.removeProperty('--tf-tilt-x');
      card.style.removeProperty('--tf-tilt-y');
      card.style.removeProperty('--tf-glow-x');
      card.style.removeProperty('--tf-glow-y');
    };
