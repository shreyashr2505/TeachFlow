...
const MessagingCenter: React.FC = () => {
  ...
  if (currentClass && !canAccessFeature('messaging', currentClass.plan)) {
    return (
      <UpgradeCard
        title="Messaging Locked"
        description="In-app messaging for admins, teachers, students, and parents unlocks on the Standard plan so class communication stays organized."
        requiredPlan="Standard"
        currentPlan={formatPlanName(currentClass.plan)}
        actionLabel={user?.role === 'admin' ? 'View Plans' : 'Upgrade Required'}
        onUpgradeClick={user?.role === 'admin' ? () => requestDashboardTabNavigation('pricing') : undefined}
      />
    );
  }
          ...
          <div className="space-y-4">
            <div>
              ...
              <StyledSelect
                ...
                onChange={(value) => setFormData((prev) => ({ ...prev, toUserId: value }))}
                options={[
                  { value: '', label: 'Select recipient' },
                  ...recipientOptions.map((item) => ({ value: item.id, label: `${item.name} (${item.role})` })),
                ]}
              />
            </div>
