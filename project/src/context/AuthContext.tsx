...
type AuthLikeError = { code?: string; message?: string };
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const refreshUserData = async () => {
    ...
    const userClasses =
      profile.role === 'admin'
        ? await firebaseService.syncAdminPlanState(profile.id)
    ...
    const fallbackActiveClass = userClasses.find((item) => item.isActive) ?? userClasses[0] ?? null;
    const resolvedCurrentClassId =
      requestedClass?.isActive !== false
        ? requestedClass?.id ?? fallbackActiveClass?.id ?? null
        : fallbackActiveClass?.id ?? null;
  ...
  useEffect(() => {
    const unsubscribe = firebaseService.subscribeToPlanSettings((settings) => {
  ...
  const login = async (email: string, password: string, tenantSlug?: string) => {
    try {
      ...
      if (tenantSlug && targetClass && profile) {
        ...
        const isClassOwner = profile.role === 'admin' && targetClass.adminId === profile.id;
      ...
      await refreshUserData();
      window.dispatchEvent(new CustomEvent('teachflow:login-success'));
      void firebaseService.trackGrowthEvent({
        type: 'login',
        source: tenantSlug ? 'tenant_login' : 'main_login',
        classId: targetClass?.id,
        userId: credential.user.uid,
        plan: profile.classIds?.length ? undefined : 'free',
      }).catch(() => undefined);
      return true;
    } catch (error: unknown) {
  ...
  const signup = async (
    ...
    try {
      ...
      void firebaseService.trackGrowthEvent({
        type: 'signup',
        source: tenantSlug ? 'tenant_signup' : 'main_signup',
        classId: targetClass?.id,
        userId: credential.user.uid,
        plan: targetClass ? undefined : 'free',
      }).catch(() => undefined);
      return true;
    } catch (error: unknown) {
